...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const priceByVal = body.priceBy !== undefined ? String(body.priceBy || '').trim().toUpperCase() : undefined;
    if (description === undefined && erpCodeVal === undefined && priceByVal === undefined) return NextResponse.json({ error: 'Nada para atualizar' }, { status: 400 });
    if (priceByVal !== undefined && !['UNIT', 'WEIGHT'].includes(priceByVal)) return NextResponse.json({ error: 'Preço Por inválido' }, { status: 400 });
    ...
    const priceByEsc = priceByVal !== undefined ? priceByVal.replace(/'/g, "''") : undefined;
    const setParts: string[] = [];
    if (esc !== undefined) setParts.push(`"description"='${esc}'`);
    if (erpEsc !== undefined) setParts.push(`"erpCode"=${erpEsc === null ? 'NULL' : `'${erpEsc}'`}`);
    if (priceByEsc !== undefined) setParts.push(`"priceBy"='${priceByEsc}'`);
    ...
    const rows: any[] = await prisma.$queryRawUnsafe(`SELECT id, description, "erpCode", "priceBy" FROM "CommercialFamily" WHERE id=${id} LIMIT 1`);
