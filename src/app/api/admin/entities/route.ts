...
export async function GET() {
  try {
    ...
    const entities = await prisma.$queryRawUnsafe(`SELECT "id", "cnpj", "name", "isActive" FROM "Entity" ORDER BY "name"`);
...
export async function POST(request: Request) {
  try {
    ...
    const exists: any[] = await prisma.$queryRawUnsafe(`SELECT "id" FROM "Entity" WHERE "cnpj"='${cnpj}'`);
    if (exists.length > 0) return NextResponse.json({ error: 'CNPJ já cadastrado' }, { status: 409 });
      await prisma.$executeRawUnsafe(`INSERT INTO "Entity" ("cnpj", "name", "isActive") VALUES ('${cnpj}', '${name.replace(/'/g, "''")}', TRUE)`);
    const row: any[] = await prisma.$queryRawUnsafe(`SELECT "id" FROM "Entity" WHERE "cnpj"='${cnpj}' LIMIT 1`);
